...
CI_Project_data = load('CI_Project_data_01.mat');
TrainLabel = CI_Project_data.TrainLabel;
TrainLabel(TrainLabel==2) = 0;
CI_Project_data.TrainLabel = TrainLabel;
save('CI_Project_data_01','CI_Project_data')
CI_Project_data = cell2mat(struct2cell(load('CI_Project_data_01.mat')));
save('CI_Project_data_01','CI_Project_data')

%% Calculate Features
clc
clear
load('CI_Project_data_01');
...
freq_name = ['E:\matlabProjects\CI_HW\project\FrequencyDomainFeatures'];
time_name = ['E:\matlabProjects\CI_HW\project\TimeDomainFeatures'];
save(freq_name,'FrequencyFeatures')
save(time_name,'TimeFeatures')
...
freq_name = ['E:\matlabProjects\CI_HW\project\FrequencyDomainFeatures_Test'];
time_name = ['E:\matlabProjects\CI_HW\project\TimeDomainFeatures_Test'];
save(freq_name,'FrequencyFeatures_Test')
save(time_name,'TimeFeatures_Test')
%% Feature Extraction
clc
clear
load('CI_Project_data_01')
...
TestData =  CI_Project_data.TestData;
TrainLabel = CI_Project_data.TrainLabel;
load('TimeDomainFeatures')
load('FrequencyDomainFeatures')
load('FrequencyDomainFeatures_Test')
load('TimeDomainFeatures_Test')

%Define Classes
Class1 = find(TrainLabel==1) ;
Class0 = find(TrainLabel==0) ; % label 2 is replaced with 0
...
number_neuron = [1,5,10,15,20,25,30,35,40,45,50,65,70,80,90];
...
for i = 1:length(number_neuron)
    ...
    accuracy(i) = accuracy(i)/(5*0.2*110);
    ...
    if i == 1
        ...
        save('MLP_OneOutput_Labels','MLP_ONE_OUTPUT_LABEL')
...
for i = 1:length(number_neuron)
    ...
    if i==1
        ...
        save('MLP_TwoOutput_Labels','MLP_TWO_OUTPUT_LABEL')
...
for i = 1:length(MN)
    for j = 1:length(spreed)
        ...
        accuracy(i,j) = accuracy(i,j)./(5*0.2*110);
        ...
        if accuracy(i,j)> M
            ...
            save('RBF_OneOutput_Labels','RBF_ONE_OUTPUT_LABEL')
...
for i = 1:length(MN)
    for j = 1:length(spreed)
        for k = 1:5
            cv = cvpartition(size(BestFeatures,2),'HoldOut',0.3);
            idx = cv.test;
        ...
        accuracy(i,j) = accuracy(i,j)./(5*0.3*110);
        ...
        if accuracy(i,j)> M
            ...
            save('RBF_TwoOutput_Labels','RBF_TWO_OUTPUT_LABEL')
...
save('LastPopulation_Test','Last_Pop_Test');
save('LastPopulation_Train','Last_Pop_Train');
...
TestData =  CI_Project_data.TestData;
TrainLabel = CI_Project_data.TrainLabel;
load('LastPopulation_Test');
load('LastPopulation_Train');
...
for i = 1:length(number_neuron)
    ...
    if i == 1
        ...
        Ev_MLP_ONE_OUTPUT_LABEL = predict_y;
        
        save('EvolutionaryMLP_OneOutput_Labels','Ev_MLP_ONE_OUTPUT_LABEL')
...
for i = 1:length(number_neuron)
    ...
    if i==1
       ...
       Ev_MLP_TWO_OUTPUT_LABEL = predict_y;
        
       save('EvolutionaryMLP_TwoOutput_Labels','Ev_MLP_TWO_OUTPUT_LABEL')
...
for i = 1:length(MN)
    for j = 1:length(spreed)
        ...
        if accuracy(i,j)> M
            ...
            Ev_RBF_ONE_OUTPUT_LABEL = predict_y;
            
            save('EvolutionaryRBF_OneOutput_Labels','Ev_RBF_ONE_OUTPUT_LABEL')
...
for i = 1:length(MN)
    for j = 1:length(spreed)
        ...
        if accuracy(i,j)> M
            ...
            Ev_RBF_TWO_OUTPUT_LABEL = predict_y;
            
            save('EvolutionaryRBF_TwoOutput_Labels','Ev_RBF_TWO_OUTPUT_LABEL')
